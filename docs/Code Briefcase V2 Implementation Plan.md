...
| ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------------------- | ------------------------- |
| `briefcase-types`              | Canonical domain types and newtypes.                         | `AgentSessionId`, `FileId`, `RepoEpochId`, `EventId`, `ContextFrameId`, enums, schema structs. | none                                                         | `delegate droid grok work` for typed schema precision | P0 substrate              |
| `briefcase-config`             | Config loading, protocol version, feature flags, ablation flags, config hash. | `BriefcaseConfig`, `ConfigHash`, `FeatureFlags`, `load_config`. | `briefcase-types`                                            | `delegate codex work`                                 | P0 substrate              |
| `briefcase-identity`           | Canonical project/worktree identity.                         | `resolve_project_identity(root, config) -> ProjectIdentity`. | `briefcase-types`, `briefcase-config`                        | `delegate codex work`                                 | P0 substrate              |
| `briefcase-file`               | File hashing, file metadata, `FileVersion`, `source_class`.  | `snapshot_file`, `hash_file`, `classify_source`, `FileVersionStore` trait. | `briefcase-types`, `briefcase-identity`                      | `delegate droid "deepseek v4 pro" work`               | P0 substrate              |
| `briefcase-vcs`                | Git state and `RepoEpoch`.                                   | `detect_repo_epoch`, `OperationState`, `WorkingTreeDigest`.  | `briefcase-types`, `briefcase-identity`, `briefcase-file`    | `delegate cursor work`                                | P0 substrate              |
| `briefcase-protocol`           | Hook events, daemon decisions, adapter capability bitset, wire schemas. | `DaemonEvent`, `DaemonDecision`, `AdapterCapabilities`, request/response envelopes. | `briefcase-types`                                            | `delegate droid grok work`                            | P0 substrate              |
| `briefcase-storage`            | Append-only event log, migrations, materialized views, credential redaction. | `EventLog`, `MaterializedViews`, `StorageTxn`, `append_event`. | `briefcase-types`, `briefcase-file`, `briefcase-vcs`, `briefcase-identity` | `delegate codex work`                                 | P0 substrate              |
| `briefcase-daemon-client`      | Thin client for adapters, CLI, MCP.                          | `DaemonClient`, `connect_or_launch`, `send_event`.           | `briefcase-protocol`, `briefcase-identity`, `briefcase-config` | `delegate droid "deepseek v4 flash" work`             | P0 substrate              |
| `briefcase-daemon`             | Single per-worktree daemon, lease, heartbeat, socket, service registry. | `Daemon`, `DaemonService`, `launch_or_attach`, health endpoints. | `briefcase-*` substrate crates                               | `delegate droid "deepseek v4 pro" work`               | P0 substrate              |
| `briefcase-ledger`             | Session registry, ObservationLedger, EditLedger, Task identity, inheritance, freshness decisions. | `record_observation`, `record_edit_intent`, `check_direct_freshness`, `inherit_observations`. | `briefcase-types`, `briefcase-storage`, `briefcase-file`, `briefcase-vcs`, `briefcase-protocol` | `delegate codex work`                                 | P0 substrate              |
| `briefcase-context`            | ContextFrame ledger, scheduler skeleton, novelty/dedup, renderable decorations. | `emit_context_frame`, `schedule_decoration`, `ContextBudget`, `ContextRenderer`. | `briefcase-types`, `briefcase-storage`, `briefcase-ledger`, later `briefcase-graph` | `delegate codex work`                                 | P0 now, P1 full scheduler |
| `briefcase-incremental`        | Salsa wrapper and hot query DAG boundary.                    | `IncrementalDb`, query keys, query invalidation API.         | `briefcase-types`, `briefcase-file`, `briefcase-config`      | `delegate droid grok work`                            | P0 substrate              |
| `briefcase-graph`              | Graph model, file inventory, symbols, deps, fingerprints, provenance. | `GraphSnapshot`, `DependencySet`, `SurfaceItem`, `FingerprintSet`, `GraphQuery`. | `briefcase-types`, `briefcase-file`, `briefcase-vcs`, `briefcase-incremental`, `briefcase-storage` | `delegate droid "deepseek v4 pro" work`               | P0 substrate              |
| `briefcase-extract-core`       | Versioned extractor contracts.                               | `Extractor`, `ExtractedFact`, `FactProvenance`, `Confidence`, grammar/language/framework traits. | `briefcase-types`, `briefcase-graph`, `briefcase-file`       | `delegate codex work`                                 | P0 substrate              |
| `briefcase-extract-p0alpha`    | TypeScript/JavaScript, Python, Go, Rust extractors.          | P0-α extractor implementations and fixture harness bindings. | `briefcase-extract-core`, `briefcase-graph`, `briefcase-incremental` | split by language workers                             | P0 substrate              |
| `briefcase-extract-frameworks` | Next.js, FastAPI, Django, Flask, Go web, Rust web framework routes. | Framework route, handler, middleware extractors.             | `briefcase-extract-core`, `briefcase-graph`                  | split by framework workers                            | P0/P1 capability          |
| `briefcase-extract-p0beta`     | Java, C#, PHP, Ruby, C/C++, Swift, Kotlin, Dart navigation-grade extractors. | P0-β extractor implementations and fixtures.                 | `briefcase-extract-core`, `briefcase-graph`                  | model-diverse extension wave                          | P1/P2 extension           |
| `briefcase-extract-bridges`    | React Native, Expo, Swift/Obj-C, JNI, FFI bridge extractors. | Bridge edge extractors and bridge provenance.                | `briefcase-extract-core`, `briefcase-graph`                  | `delegate droid qwen work` plus reviewers             | P2 extension              |
| `briefcase-diagnostics`        | Persistent diagnostic workers, caches, before/after deltas.  | `DiagnosticWorker`, `DiagnosticDelta`, `DiagnosticCache`, `attribute_delta`. | `briefcase-types`, `briefcase-file`, `briefcase-storage`, `briefcase-graph`, `briefcase-metrics` | `delegate droid "deepseek v4 pro" work`               | P0 capability             |
| `briefcase-metrics`            | Local metrics ledger, latency, token/tool counters, VAT, ablations. | `MetricsLedger`, `VatScore`, `AblationSet`, `LatencySample`. | `briefcase-types`, `briefcase-storage`, `briefcase-protocol` | `delegate codex work`                                 | P1 product feature        |
| `briefcase-bench`              | Benchmark harness and scenario runner.                       | `BenchmarkRun`, `Scenario`, `AgentHarness`, result bundles.  | `briefcase-metrics`, `briefcase-harness-sim`, `briefcase-daemon-client` | `delegate cursor work` with Codex review              | P1 product feature        |
| `briefcase-mcp`                | MCP server with seven tools, staged delivery.                | `briefcase_orient`, `briefcase_observed_state`, `briefcase_prove`, later `find/explain/impact/diagnostics`. | `briefcase-daemon-client`, `briefcase-context`, `briefcase-ledger`, `briefcase-graph`, `briefcase-diagnostics` | `delegate codex work`                                 | P0/P1 integration         |
| `briefcase-adapter-core`       | Shared adapter discovery, launch, spool, event normalization. | `AdapterRuntime`, `Spool`, `CapabilityRegistration`, harness-normalized events. | `briefcase-protocol`, `briefcase-daemon-client`, `briefcase-identity` | `delegate codex work`                                 | P0 integration            |
| `briefcase-adapter-claude`     | Claude Code Tier 1 hook adapter.                             | Claude hook entrypoints and capability registration.         | `briefcase-adapter-core`                                     | `delegate codex work`                                 | P0 integration            |
| `briefcase-adapter-codex`      | Codex CLI adapter, Tier 1 if deny/precondition support exists, otherwise Tier 2. | Codex hook/MCP/client shim.                                  | `briefcase-adapter-core`                                     | `delegate codex work`                                 | P0/P1 integration         |
| `briefcase-adapter-cursor`     | Cursor adapter, expected Tier 2.                             | Cursor shim and event normalization.                         | `briefcase-adapter-core`                                     | `delegate cursor work`                                | P0/P1 integration         |
| `briefcase-harness-sim`        | Harness simulator for deterministic multi-agent tests.       | `SimHarness`, event scripts, injected race/stale scenarios.  | `briefcase-protocol`, `briefcase-daemon-client`, `briefcase-adapter-core` | `delegate droid "deepseek v4 flash" work`             | P0 test substrate         |
| `briefcase-cli`                | Operator CLI.                                                | `briefcase status`, `daemon doctor`, `sessions`, `ledger`, `context`, `deny`, `graph`, `diagnostics`, `metrics`, `replay`. | `briefcase-daemon-client`, `briefcase-storage`, `briefcase-ledger`, `briefcase-context`, `briefcase-diagnostics`, `briefcase-metrics` | `delegate cursor work`                                | P0/P1 visibility          |
| `briefcase-web`                | Local read-only web UI.                                      | Local HTTP server, timeline API, session lanes UI.           | `briefcase-storage`, `briefcase-metrics`, `briefcase-ledger`, `briefcase-diagnostics` | `delegate droid gemini work`                          | P1 extension              |
| `briefcase-app`                | Single binary packaging daemon, CLI, MCP, adapter helpers.   | `briefcase` binary with subcommands.                         | all shipped crates                                           | orchestrator direct integration                       | P0 substrate              |
...
| event log/session lifecycle | Phase 1 | `crates/briefcase-storage/tests/**`, `tests/phase1_daemon.rs` | monotonic event IDs, crash-safe append, concurrent launch fencing |
| observations/edit freshness | Phase 2 | `crates/briefcase-ledger/tests/**`, `tests/phase2_stale_direct.rs` | read V1, other session writes V2, pre-edit denies/warns      |
...
| context frames              | Phase 3 | `crates/briefcase-context/tests/**`                          | all decorations have ContextFrame IDs, budgets, source versions |
...
Claude Code reaches Tier 1 first because it is the primary harness and has the strongest hook surface. Codex and Cursor begin as adapter-design tasks in parallel, but their implementation follows the stable `briefcase-adapter-core` and daemon protocol. Codex is targeted for Tier 1 if deny/precondition support is available; otherwise it ships Tier 2 with honest capability surfacing. Cursor is expected Tier 2 first. The adapter capability matrix is product surface, not internal trivia.

Adapter order:

1. `briefcase-adapter-core`: daemon discovery, launch, spool, capability registration.
2. `briefcase-adapter-claude`: Tier 1, pre-edit deny, read decoration, command replacement, PreCompact.
3. `briefcase-adapter-codex`: Tier 1 if supported, otherwise Tier 2.
4. `briefcase-adapter-cursor`: Tier 2, post-hoc observation plus advisory where possible.
...
| fixture extraction    | Phases 4+                  | `cargo test -p briefcase-extract-* --features fixtures`      |
| latency POC           | Phases 4+                  | `cargo bench -p briefcase-bench latency_poc -- --quick`      |
| multi-agent scenarios | Phases 2+                  | `cargo test -p briefcase-harness-sim`                        |
| packaging smoke       | Phases 5+                  | `cargo run -p briefcase-app -- daemon doctor --self-test`    |
...
- Case sensitivity is modeled in `briefcase-identity`.
...
Compiler-backed or tool-backed extraction is a hard requirement for facts that can drive hard-deny enforcement. Per the spec's confidence/provenance model, a fact may only justify a hard deny if its provenance includes a compiler-backed or tool-backed source. The required seams documented in `briefcase-extract-core`:
...
Grammar version pinning: every extractor records its grammar version (tree-sitter grammar SHA or compiler-backed tool version) in `ExtractedFact.provenance`. The Salsa query key in `briefcase-incremental` includes the grammar version so a grammar bump invalidates exactly the affected facts.
...
- `briefcase` binary starts daemon and `briefcase daemon doctor --self-test` passes.
...
- Owned files: `Cargo.toml`, `rust-toolchain.toml`, `.cargo/config.toml`, `crates/briefcase-app/**`, `docs/dev/gates.md`, `CLAUDE.md`.
- Dependencies within wave: none.
...
Task 2: `briefcase-types` schemas and IDs.
...
- Owned files: `crates/briefcase-types/**`.
- Dependencies within wave: none.
...
Task 3: `briefcase-config` config hash and feature flags.

- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-config/**`, `fixtures/config/**`.
- Dependencies within wave: none.
...
Task 4: `briefcase-identity` project/worktree identity.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `using-git-worktrees`.
- Owned files: `crates/briefcase-identity/**`, `fixtures/identity/**`.
- Dependencies within wave: none.
...
Task 5: `briefcase-file` FileVersion and source-class base.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-file/**`, `fixtures/files/**`.
- Dependencies within wave: none.
...
Task 6: `briefcase-vcs` RepoEpoch.

- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `using-git-worktrees`.
- Owned files: `crates/briefcase-vcs/**`, `fixtures/vcs/**`.
- Dependencies within wave: none.
...
Task 1: `briefcase-storage` append-only event log.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-storage/**`, `crates/briefcase-storage/migrations/**`.
- Dependencies within wave: none, uses `briefcase-types` public contract from Wave 1.1.
...
Task 2: `briefcase-protocol` daemon event and decision envelopes.

- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-protocol/**`.
- Dependencies within wave: none.
...
Task 3: `briefcase-daemon-client` connect or launch.

- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `using-git-worktrees`.
- Owned files: `crates/briefcase-daemon-client/**`.
- Dependencies within wave: none.
...
Task 5: `briefcase-cli` minimal status and doctor.

- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-cli/**`.
- Dependencies within wave: none.
- Acceptance: `briefcase status` and `briefcase daemon doctor --self-test` call daemon client, show identity, daemon generation, storage path, and degraded state.
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
...
Task 6: `briefcase-harness-sim` Phase 1 daemon fixture.

- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-harness-sim/**`, `tests/phase1_daemon.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** `AdapterCapabilities` bitset finalized in `briefcase-protocol` with all P0/P1/P2 capability bits enumerated, and `CapabilityRegistration` event variant added to `DaemonEvent`. Capability view query added to `Daemon` trait. Workers then implement against the stable bitset; no late-bound capability discovery.

Task 1: `briefcase-app` binary wiring.

- Owner: orchestrator direct.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `finishing-a-development-branch`; secondary `checkpoint`.
- Owned files: `crates/briefcase-app/**`, root `Cargo.toml`.
- Dependencies within wave: none.
- Acceptance: `briefcase daemon`, `briefcase status`, `briefcase daemon doctor --self-test` invoke the right crates; root gate runs.
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-daemon/**`, `crates/briefcase-protocol/**`, `tests/phase1_capabilities.rs`.
- Dependencies within wave: none.
...
- Phase gate: `cargo fmt --check`; `cargo clippy --workspace --all-targets --no-deps`; `cargo test --workspace`; `briefcase daemon doctor --self-test`; `cargo test --test phase1_concurrent_launch`; event-log golden fixture replay.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-types`: `AgentSessionId`, `TaskId`, `ObservationId`, `EditIntentId`, `EditAppliedId`, `SessionLineage` (parent_id, root_id, depth). In `briefcase-protocol`: typed envelopes for `SessionStart` (with lineage fields), `ReadObserved`, `EditIntent`, `EditApplied`. In `briefcase-ledger`: public trait surface for `record_observation`, `record_edit_intent`, `record_edit_applied`, `record_session_lifecycle` — bodies stubbed with `todo!()`. In `briefcase-storage`: materialized-view trait skeletons for sessions, observations, edit attempts, current file versions. CLI and daemon consume the trait surfaces; ledger and storage workers fill in implementations.

Task 1: `briefcase-ledger` session, task, observation, edit APIs.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-ledger/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-daemon/**`, `tests/phase2_ingestion.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-cli/**`, `tests/phase2_cli_sessions.rs`.
- Dependencies within wave: none.
- Acceptance: `briefcase sessions list`, `briefcase sessions show`, `briefcase observations show`, and `briefcase ledger tail --session` work against materialized views.
- Reviewer pairing: `delegate droid "deepseek v4 pro" safe` with `rust-engineer`, `clean-code`.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-protocol`: `expected_target_file_hash`, `adapter_precondition_support`, `decision_id`, `observed_previous_hash` fields on `EditIntent`, `DaemonDecision`, and `EditApplied`. In `briefcase-types`: `FreshnessDecision` enum (Allow, Advisory, Deny) with revalidation-set payload. In `briefcase-ledger`: `check_direct_freshness(session, target_file) -> FreshnessDecision` trait signature. In `briefcase-daemon`: PreEdit endpoint request/response types. Adapter-core consumes the precondition fields; ledger and daemon fill in decision logic.

Task 1: Direct file freshness decision engine.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/**`, `tests/phase2_direct_freshness.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-daemon/**`, `tests/phase2_preedit.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `using-git-worktrees`.
- Owned files: `crates/briefcase-adapter-core/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-storage/**`, `crates/briefcase-daemon/**`, `tests/phase2_toctou.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-types`: `InheritedObservation` (with origin_session_id, original_observation_id, inheritance_depth), `SubagentOrientationPacket` (task slice, parent working set, inherited observations, stale risks, ContextFrame ID references, suggested reads), `DenyDecision` (deny_id, minimum_revalidation_set, proof_availability, override_policy), `DenyLoopGuard`. In `briefcase-protocol`: child-session spawn event. In `briefcase-ledger`: `inherit_observations(child, parent)` trait signature, deny-loop-guard query. Daemon consumes the spawn event; ledger and context (Phase 3 boundary) consume the orientation packet types.

Task 1: Subagent inheritance in `briefcase-ledger`.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-ledger/**`, `tests/phase2_subagent_inheritance.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-context/**`, `crates/briefcase-types/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/**`, `crates/briefcase-protocol/**`, `tests/phase2_deny_recovery.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-daemon/**`, `tests/phase2_child_spawn.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `using-git-worktrees`.
- Owned files: `crates/briefcase-harness-sim/**`, `tests/phase2_multi_agent_direct.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-types`: `ContextFrameId`, `ContextFrame` struct (facts, source_versions, token_count, confidence, expiry, novelty_hash, utility_receipt placeholder), `TokenBudget` per surface, `ContextRenderFormat` enum. In `briefcase-context`: public trait `ContextRenderer` (human + JSON), trait `emit_context_frame`, trait `TokenEstimator`. In `briefcase-storage`: context dedup materialized-view trait. CLI consumes the render and CFID query surface; storage worker fills in the dedup view.

Task 1: ContextFrame storage and API.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-context/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/token_budget.rs`, `crates/briefcase-context/tests/token_budget.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/render/**`, `crates/briefcase-context/tests/render_golden.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work --isolation worktree`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-cli/src/context.rs`, `tests/phase3_context_cli.rs`.
- Dependencies within wave: none.
- Acceptance: `briefcase context show <context_frame_id>` displays facts, source versions, confidence, triggering tool call, token count, expiry.
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-context`: `schedule_decoration(surface, session, payload) -> ScheduleDecision` trait. In `briefcase-protocol`: `CompactionCheckpoint` event variant with pre-compact frames, promoted/omitted fact IDs, working-set snapshot. In `briefcase-types`: `context_frame_id: Option<ContextFrameId>` field added to `Observation`. Subagent orientation packet emitter consumes the packet type stubbed in Wave 2.3.

Task 1: Template scheduler skeleton.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-context/src/scheduler/**`, `crates/briefcase-context/tests/scheduler_skeleton.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/**`, `tests/phase3_observation_context.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-protocol/**`, `crates/briefcase-context/**`, `tests/phase3_precompact.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/subagent.rs`, `tests/phase3_subagent_packet.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-protocol`: `context_frame_ids: Vec<ContextFrameId>` field added to `DaemonDecision`. `DegradedContextNotice` payload type. In `briefcase-context`: replay trait surface for context-frame stream reconstruction. Benchmark crate consumes the replay trait.

Task 1: DaemonDecision context-frame integration.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-daemon/**`, `tests/phase3_daemon_context.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/degraded.rs`, `crates/briefcase-protocol/**`, `tests/phase3_degraded_context.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-storage/**`, `crates/briefcase-context/tests/replay.rs`.
- Dependencies within wave: none.
...
In `briefcase-incremental`: `IncrementalDb` trait, `QueryKey` struct (must include `content_hash`, `config_hash`, `tool_version`, `extractor_version`, `grammar_version`), `QueryInvalidation` API, Salsa version pinned and re-exported behind crate boundary so no other crate imports Salsa directly.

In `briefcase-graph`: `GraphSnapshot` (versioned), `GraphVersion`, `SurfaceItem` (per spec Appendix B fields), `DependencySet` (file-level and per-exported-surface), `FingerprintSet` (contract + implementation), `SymbolOccurrence`, `SymbolLineageId`, `Confidence` enum (High/Medium/Low), `FactProvenance` (extractor_id, extractor_version, grammar_version, input_hash, source_span, tool_backed: bool), `GraphQuery` trait surface (no implementation — only signatures), `prove_fact(fact_id) -> ProofTrace` trait method, file inventory types, partial-coverage markers, generated/vendored source-class hook points.

In `briefcase-extract-core`: `Extractor` trait (per language), `Resolver` trait, `GrammarAdapter` trait, `FrameworkExtractor` trait, `BridgeExtractor` trait, `ExtractedFact` struct embedding `FactProvenance` and `Confidence`, `ExtractorVersion` newtype. The compiler-backed-seam adapter slots from §3.7 are declared here as optional trait associated types.

In `briefcase-bench`: `LatencyPocScenario` trait + `LatencyReport` struct.

In `briefcase-ledger`: trait signature for `check_dependency_freshness(session, target_file, graph_snapshot)` added — implementation lives in Wave 4.4. This signature must exist now so adapters and MCP in Phase 5 can reason about it without churn.

Belief-management discipline check (the §14 design pin Pro flagged): no method on `GraphQuery` should return a "current truth" answer that bypasses `briefcase-ledger`'s observation/edit semantics. Graph facts are inputs to ledger decisions, not the primary API. The reviewer for Tasks 2 and 4 must verify this explicitly.

Task 1: `briefcase-incremental` Salsa boundary.

- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-incremental/**`.
- Dependencies within wave: none.
...
Task 2: `briefcase-graph` graph model and provenance.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-graph/**`.
- Dependencies within wave: none.
...
Task 3: `briefcase-extract-core` versioned extractor contracts.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-core/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-graph/src/inventory.rs`, `crates/briefcase-graph/tests/inventory.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/**`, `benches/latency_poc.rs`, `fixtures/latency/**`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0alpha/src/typescript.rs`, `fixtures/p0alpha/typescript/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0alpha/src/python.rs`, `fixtures/p0alpha/python/**`.
- Dependencies within wave: none.
...
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0alpha/src/go.rs`, `fixtures/p0alpha/go/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0alpha/src/rust.rs`, `fixtures/p0alpha/rust/**`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-extract-p0alpha/tests/**`, `fixtures/p0alpha/expected/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-extract-p0alpha/src/python_fingerprint.rs`, `fixtures/p0alpha/python_fingerprint/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-extract-p0alpha/src/go_fingerprint.rs`, `fixtures/p0alpha/go_fingerprint/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-extract-p0alpha/src/rust_fingerprint.rs`, `fixtures/p0alpha/rust_fingerprint/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-graph/src/dependencies.rs`, `crates/briefcase-graph/tests/dependencies.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-ledger`: full body for the `check_dependency_freshness` signature stubbed in Wave 4.1, plus `MinimumRevalidationSet` computation trait. In `briefcase-context`: `GraphReadDecoration` payload schema (outline, imports/exports, adjacent files, callers/tests placeholders, graph version, provenance, coverage marker, token budget) added to the context renderer surface. In `briefcase-graph`: `ProofTrace` struct finalized (source spans, versions, extractor provenance, confidence, input hashes, invalidation status) — the same surface `briefcase_prove` MCP will consume in Phase 5.

Task 1: Graph dependency staleness in ledger.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/**`, `tests/phase4_dep_staleness.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/graph_decorations.rs`, `tests/phase4_read_decoration.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/src/scenarios/dep_stale.rs`, `fixtures/bench/dep_stale/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-graph/src/prove.rs`, `crates/briefcase-graph/tests/prove.rs`.
- Dependencies within wave: none.
...
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
- Review brief shape: verify proof can back `briefcase_prove` without exposing operator-only internals.
...
Goal: make hooks and MCP thin clients of the daemon, with Claude Code first to Tier 1, Codex and Cursor in parallel once adapter-core is stable, and MCP shipping the first three tools: `briefcase_orient`, `briefcase_observed_state`, and `briefcase_prove`. This phase proves that multi-agent coordination works end-to-end through at least two adapters.
...
- MCP tools `briefcase_orient`, `briefcase_observed_state`, `briefcase_prove` work against daemon state.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-adapter-core`: `AdapterRuntime` trait (daemon discovery, launch, spool, capability registration, event normalization, timeout/fail-open policy), `Spool` trait with idempotency keys, `CapabilityProfile` per tier. In `briefcase-mcp`: `McpTool` trait, `McpToolRegistry`, tool envelope types for the three Phase 5 tools stubbed. In `briefcase-harness-sim`: `SimAdapter` trait that can emulate Tier 1, Tier 2, and Tier 3 profiles. Real adapters and the simulator implement against the same `AdapterRuntime` surface — no harness-specific state lives in core.
...
- Owned files: `crates/briefcase-adapter-claude/**`, `adapters/claude/**`, `tests/phase5_claude_adapter.rs`.
- Dependencies within wave: none.
...
- Owned files: `crates/briefcase-mcp/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-harness-sim/**`, `tests/phase5_adapter_conformance.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-cli/src/capabilities.rs`, `docs/capability-matrix.md`.
- Dependencies within wave: none.
- Acceptance: `briefcase status --capabilities` shows Tier 1/2/3 by adapter and exact bitset; docs match command output.
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-adapter-core`: `PreCompactBridge` trait (how a capable adapter emits `CompactIntent` and receives the checkpoint frame). In `briefcase-harness-sim`: multi-adapter scenario harness skeleton with capability-tier-aware assertions. Spool replay test surface for adapter crash injection. Codex and Cursor adapters implement against the same `AdapterRuntime` from Wave 5.1; PreCompact integration touches Claude adapter and adapter-core together.
...
- Owned files: `crates/briefcase-adapter-codex/**`, `adapters/codex/**`, `tests/phase5_codex_adapter.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-adapter-cursor/**`, `adapters/cursor/**`, `tests/phase5_cursor_adapter.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-adapter-core/**`, `crates/briefcase-adapter-claude/**`, `tests/phase5_precompact_adapter.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `tests/phase5_multi_adapter.rs`, `crates/briefcase-harness-sim/src/scenarios/multi_adapter.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 flash" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `tests/phase5_spool_replay.rs`, `crates/briefcase-adapter-core/tests/spool.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-mcp`: tool envelope schemas for `briefcase_orient` (task-aware project orientation request/response), `briefcase_observed_state` (direct + inherited observation query response), and `briefcase_prove` (fact_id / context_frame_id / deny_id / diagnostic_id / graph_edge_id discriminated input, `ProofTrace` response). In `briefcase-context`: `CommandIntercept` decision type (allow, structured-answer, pass-through). Hook latency benchmark trait pre-committed so all four tool benchmarks share the same instrumentation.

Task 1: `briefcase_orient`.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `tdd-workflow`.
- Owned files: `crates/briefcase-mcp/src/tools/orient.rs`, `tests/phase5_mcp_orient.rs`.
- Dependencies within wave: none.
...
Task 2: `briefcase_observed_state`.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `tdd-workflow`.
- Owned files: `crates/briefcase-mcp/src/tools/observed_state.rs`, `tests/phase5_mcp_observed.rs`.
- Dependencies within wave: none.
...
Task 3: `briefcase_prove`.

- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-mcp/src/tools/prove.rs`, `tests/phase5_mcp_prove.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-context/src/command_intercept.rs`, `crates/briefcase-adapter-core/**`, `tests/phase5_bash_intercept.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/src/hook_latency.rs`, `tests/phase5_hook_latency.rs`.
- Dependencies within wave: none.
...
- `briefcase_find`, `briefcase_explain`, `briefcase_impact`, `briefcase_diagnostics` work.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-diagnostics`: `DiagnosticWorker` trait, `DiagnosticProfile`, `DiagnosticSnapshot`, `DiagnosticDelta` (introduced/resolved/changed/full modes), `attribute_delta` trait signature, `DiagnosticCacheKey` (file_hash + dep_hash + config_hash + tool_version). In `briefcase-mcp`: `briefcase_diagnostics` tool envelope. All four diagnostic adapter workers (TS/Python and Go/Rust) implement the same trait surface; the delta attribution engine consumes snapshots without needing language-specific branching.

Task 1: Diagnostic worker registry.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-diagnostics/**`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-diagnostics/src/adapters/ts.rs`, `crates/briefcase-diagnostics/src/adapters/python.rs`, `fixtures/diagnostics/ts_py/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-diagnostics/src/adapters/go.rs`, `crates/briefcase-diagnostics/src/adapters/rust.rs`, `fixtures/diagnostics/go_rust/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-diagnostics/src/delta.rs`, `crates/briefcase-diagnostics/tests/delta.rs`.
- Dependencies within wave: none.
...
Task 5: `briefcase_diagnostics` MCP tool.

- Owner: `delegate codex work --isolation worktree`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `tdd-workflow`.
- Owned files: `crates/briefcase-mcp/src/tools/diagnostics.rs`, `tests/phase6_mcp_diagnostics.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-frameworks/src/python_web.rs`, `fixtures/frameworks/python_web/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-frameworks/src/go_web.rs`, `fixtures/frameworks/go_web/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-frameworks/src/rust_web.rs`, `fixtures/frameworks/rust_web/**`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-extract-frameworks/tests/**`, `fixtures/frameworks/expected/**`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-mcp`: tool envelopes for `briefcase_find` (kind/scope/confidence filter, results with provenance), `briefcase_explain` (callers, callees, tests, routes, depth, token budget, provenance), `briefcase_impact` (impact radius, dependency changes, affected symbols, affected sessions, stale observations — note: affected sessions derives from ledger, not graph-only guesses). Streaming/summarizing fallback trait so `briefcase_explain` has a single token-budget enforcement path.

Task 1: `briefcase_find`.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `tdd-workflow`.
- Owned files: `crates/briefcase-mcp/src/tools/find.rs`, `tests/phase6_mcp_find.rs`.
- Dependencies within wave: none.
...
Task 2: `briefcase_explain`.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `tdd-workflow`.
- Owned files: `crates/briefcase-mcp/src/tools/explain.rs`, `tests/phase6_mcp_explain.rs`.
- Dependencies within wave: none.
...
Task 3: `briefcase_impact`.

- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `mcp-builder`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-mcp/src/tools/impact.rs`, `tests/phase6_mcp_impact.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/src/mcp_latency.rs`, `tests/phase6_mcp_latency.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-cli`: full command surface enumerated as a typed enum so doctor and explanation polish tasks bind to stable command names. In `briefcase-file`/`briefcase-graph`: `SourceClass` policy enum (Generated, Vendored, BuildArtifact, Config, Lockfile, Migration, Fixture, Source) + classification rules trait. In `briefcase-context`: `SurvivalPacket` schema (task summary, working set, observed versions, edited files, unresolved diagnostics/stale warnings, other-agent changes, key symbols/routes, frame IDs, suggested resume calls).

Task 1: Full operator CLI command set.

- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-cli/**`, `tests/phase6_cli_full.rs`.
- Dependencies within wave: none.
...
- Owned files: `crates/briefcase-daemon/src/doctor.rs`, `tests/phase6_doctor_faults.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-file/src/source_class.rs`, `crates/briefcase-graph/src/source_policy.rs`, `tests/phase6_source_class.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `plain-language`.
- Owned files: `crates/briefcase-context/src/precompact_survival.rs`, `tests/phase6_precompact_survival.rs`.
- Dependencies within wave: none.
...
- Owned files: `crates/briefcase-cli/src/deny.rs`, `crates/briefcase-cli/src/context.rs`, `tests/phase6_explain_cli.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-metrics`: `MetricsLedger` trait, `VatScore`, `AblationSet`, `LatencySample`, full counter set enumerated (tokens, tool calls, hook latency, graph query latency/cache hits, diagnostics, stale denies/warnings, session overlap windows, final success, failure labels). In `briefcase-bench`: `BenchmarkRun` schema, `Scenario` trait, `AgentHarness` trait, ablation flag enum. In `briefcase-context`: scheduler novelty-scoring interface (utility receipt capture + duplicate-token-ratio measurement) — but the actual scoring algorithm body is Task 3's deliverable.

Task 1: Metrics ledger and VAT.

- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-metrics/**`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/**`, `tests/phase6_bench.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-context/src/scheduler/**`, `tests/phase6_context_scheduler.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-protocol`: `BroadcastEvent` enum variants for working-set-overlap notice (without runtime emission). In `briefcase-ledger`: `WorkingSet` materialized-view schema and `working_set_field` on session record. In `briefcase-metrics`: broadcast metrics counter slots. In `briefcase-config`: `broadcasts_enabled: bool` defaulting to false. The full P0 scenario bundle task consumes all five Phase 6 surfaces (diagnostics, frameworks, MCP, CLI, metrics) plus the inert broadcast placeholders.

Task 1: Inert broadcast scaffolding (placeholders only, no runtime loop).

- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-protocol/src/broadcasts.rs` (event-kind enums and message types only), `crates/briefcase-ledger/src/working_set.rs` (working-set materialized view), `crates/briefcase-metrics/src/broadcasts.rs` (metrics slots), `crates/briefcase-config/src/broadcasts.rs` (disabled-by-default config flag), `tests/phase6_broadcast_placeholders.rs`.
- Dependencies within wave: none.
...
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0beta/src/csharp.rs`, `fixtures/p0beta/csharp/**`.
- Dependencies within wave: none.
...
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0beta/src/php_ruby.rs`, `fixtures/p0beta/php_ruby/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-p0beta/src/native_outlines.rs`, `fixtures/p0beta/native/**`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-extract-p0beta/tests/**`, `fixtures/p0beta/expected/**`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-web`: read-only HTTP API schema (session lanes, events, context frames, denies, diagnostics, graph invalidations, VCS epoch changes — all GET endpoints, no writes). Static asset entry-point convention. The UI implementation, daemon integration, and usability benchmark all consume the API schema; no separate state lives in `briefcase-web`.
...
- Owned files: `crates/briefcase-web/src/api/**`, `tests/phase7_web_api.rs`.
- Dependencies within wave: none.
...
- Owned files: `crates/briefcase-web/static/**`, `crates/briefcase-web/src/ui/**`.
- Dependencies within wave: none.
...
- Owned files: `crates/briefcase-daemon/src/web.rs`, `crates/briefcase-app/**`, `tests/phase7_web_integration.rs`.
- Dependencies within wave: none.
- Acceptance: `briefcase web` starts local read-only server bound to localhost; status command prints URL; no network exposure by default.
- Reviewer pairing: `delegate codex safe` with `rust-engineer`, `clean-code`.
...
- Owned files: `crates/briefcase-bench/src/web_ui.rs`, `tests/phase7_web_ui.rs`.
- Dependencies within wave: none.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-identity`: `RepoIdentity` (canonical repo root above worktree level) for sibling-worktree detection. In `briefcase-graph`: `PublicCacheKey` (package version + content hash) and privacy boundary trait. In `briefcase-ledger`: `SymbolStaleness` query trait and `Lease` trait (file/symbol lease with timeout recovery, advisory fallback). The active-broadcasts runtime task (Task 5) consumes the broadcast event types and working-set storage from Phase 6 Wave 6.5b; no further protocol additions needed.

Task 1: Worktree federation advisory.

- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-daemon/src/worktree_federation.rs`, `crates/briefcase-identity/src/repo_identity.rs`, `tests/phase7_worktree_federation.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate codex work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-graph/src/public_cache.rs`, `crates/briefcase-config/src/cache.rs`, `tests/phase7_public_cache.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid grok work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/src/symbol_staleness.rs`, `crates/briefcase-graph/src/symbol_impact.rs`, `tests/phase7_symbol_stale.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-ledger/src/leases.rs`, `crates/briefcase-daemon/src/leases.rs`, `tests/phase7_leases.rs`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `debugging-systematic`.
- Owned files: `crates/briefcase-daemon/src/broadcasts.rs`, `crates/briefcase-ledger/src/working_set_listener.rs`, `tests/phase7_broadcasts.rs`.
...
**Wave-internal contracts (orchestrator commits before fan-out).** In `briefcase-extract-bridges`: `BridgeExtractor` trait specializations and `BridgeEdge` provenance struct. Per §3.7 confidence policy: bridge facts default to medium confidence and cannot alone drive hard deny. Pre-fan-out the orchestrator commits the bridge-fixture schema. The extension benchmark bundle and final integration tasks consume the existing benchmark and metrics surfaces — no new shared types needed.

Task 1: React Native and Expo bridge extractor.

- Owner: `delegate droid qwen work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-bridges/src/react_native.rs`, `fixtures/bridges/react_native/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid "deepseek v4 pro" work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-bridges/src/swift_objc.rs`, `fixtures/bridges/swift_objc/**`.
- Dependencies within wave: none.
...
- Owner: `delegate droid gemini work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `spec-quality-checklist`.
- Owned files: `crates/briefcase-extract-bridges/src/jni_ffi.rs`, `fixtures/bridges/jni_ffi/**`.
- Dependencies within wave: none.
...
- Owner: `delegate cursor work`.
- Skill stack: always-on `clean-code`, `rust-engineer`; primary `tdd-workflow`; secondary `delegate-agent`.
- Owned files: `crates/briefcase-bench/src/extension/**`, `tests/phase7_extension_bundle.rs`.
- Dependencies within wave: none.
...
2. **Wave 1.1 bootstrap prelude** — orchestrator commits the workspace skeleton in a single pre-fan-out commit: root `Cargo.toml` with workspace members, `rust-toolchain.toml`, `.cargo/config.toml`, empty crate directories for `briefcase-types`, `briefcase-config`, `briefcase-identity`, `briefcase-file`, `briefcase-vcs`, `briefcase-app`, with compile-only `lib.rs`/`main.rs` skeletons so `cargo check --workspace` passes against the empty tree. Workers then operate on their owned crate without colliding on workspace-file edits.
3. **Wave 1.1 dispatch** — orchestrator dispatches six workers in one batch.

Literal first wave dispatch:

1. `delegate codex work`
   Brief: flesh out `briefcase-app` binary scaffolding, gate documentation, and project `CLAUDE.md`. The empty workspace already builds from the bootstrap commit. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `bootstrap`, `write-human`. Own `crates/briefcase-app/**`, `docs/dev/gates.md`, `CLAUDE.md` (and minor edits to root `Cargo.toml` if app needs new dependencies). Acceptance: `briefcase-app` builds and produces the `briefcase` binary stub; `cargo fmt --check`; `cargo check --workspace`; `CLAUDE.md` includes CPU discipline (workers do not run heavy gates), Git discipline (workers do not commit, no `git add -A`), and the phase-gate command list.
2. `delegate droid grok work`
   Brief: implement `briefcase-types`. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `spec-quality-checklist`. Own `crates/briefcase-types/**`. Acceptance: typed IDs, schemas, enums, serde round trips, no stringly public IDs.
3. `delegate droid "deepseek v4 flash" work`
   Brief: implement `briefcase-config`. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `spec-quality-checklist`. Own `crates/briefcase-config/**`, `fixtures/config/**`. Acceptance: deterministic config hash, protocol version, modes, ablation flags.
4. `delegate codex work`
   Brief: implement `briefcase-identity`. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `using-git-worktrees`. Own `crates/briefcase-identity/**`, `fixtures/identity/**`. Acceptance: canonical root/git common dir/worktree/config/protocol identity with symlink and case-sensitivity tests.
5. `delegate droid "deepseek v4 pro" work`
   Brief: implement `briefcase-file`. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `spec-quality-checklist`. Own `crates/briefcase-file/**`, `fixtures/files/**`. Acceptance: content-addressed FileVersion, source-class base, large-file streaming hash tests.
6. `delegate cursor work`
   Brief: implement `briefcase-vcs`. Load `clean-code`, `rust-engineer`, `tdd-workflow`, `using-git-worktrees`. Own `crates/briefcase-vcs/**`, `fixtures/vcs/**`. Acceptance: RepoEpoch and operation_state detection for normal/detached/merge/rebase/cherry-pick/bisect/unknown.
...
Salsa lock-in: Salsa is wrapped behind `briefcase-incremental`; graph/extractor crates do not import Salsa directly.
